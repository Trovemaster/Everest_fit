...
subroutine fitting_energies
  ...
  logical                     ::  SYSTEMQQ  ! system function for  calling extyernal programs, 
   ...
   !
   allocate(Nstates(0:Nmax,2),stat=alloc)
   !
   allocate(calc(0:Nmax,2),stat=alloc)
   !
   ...
   outer_loop: do while (still_run)  
      ...
      do while( fititer.le.fitting%itermax .and. stadev.ge.stadev_best.and. stability.ge.stab_best)
        ...
        do nrow = 1,fitting%Nenergies
          ...
          !
          if (iener>Nstates(Nrot,ipar_)) then 
             enercalc(nrow) = 0
          else
             enercalc(nrow) = calc(Nrot,ipar_)%energy(iener)-ezero_
          endif
          !
          eps(nrow) = fitting%obs(nrow)%energy-enercalc(nrow)
          !
          write (f_out,"(4i5,' ',3f13.4,2x,e9.2,5x,3(i3))") &
             nrow,iener,Nrot,ipar_,enercalc(nrow)+eps(nrow),enercalc(nrow),-eps(nrow),&
           ...
           do ipar_ = 1,2 
              ...
              do irow = 1,Nstates(Nrot,ipar_)
                ...
                if (nrow<fitting%Nenergies) then
                   ...
                   !
                   write(f_en,"(2i5,f8.1,i5,' ',3f13.4,2x,e9.2,2x,a1,2x,3(i3))") &
                   ...
                   !
                   write(f_en,"(2i5,f8.1,i5,' ',3f13.4,2x,e9.2,5x,3(i3))") irow,0,Jrot,ipar_,0.0,&
                                   calc(Nrot,ipar_)%energy(irow)-ezero_,0.0,0.0
                   !
                endif
                !
              enddo
        ...
        do nrow = 1,fitting%Nenergies
          ...
          eps(nrow) = fitting%obs(nrow)%energy-enercalc(nrow)
          !
          write (f_res,"(4i5,' ',3f13.4,2x,e9.2,5x,3(i3))") &
             nrow,iener,Nrot,ipar_,enercalc(nrow)+eps(nrow),enercalc(nrow),-eps(nrow),&
          wtall(nrow),fitting%obs(nrow)%quanta%v1,fitting%obs(nrow)%quanta%v2,fitting%obs(nrow)%quanta%v3
          !      
        enddo
...
6553   format(/3X,80('-')/'   |  Iter  | Points | Params |   Deviat    |',&
  ...
  contains 
    ...
    subroutine finite_diff_of_energy(rjacob,Nstates,potparam)
      ...
      integer(ik),intent(inout)  :: Nstates(0:Nmax,2)
      real(rk),allocatable :: enerleft(:),enerright(:)
      type(calcT) :: calc_(0:Nmax,2)
      ...
      do irot = 0,Nmax
        do ipar_ = 1,2
          !
          Nsize = max(1,Nstates(irot,ipar_))
          !
          allocate(calc_(irot,ipar_)%energy(Nsize),stat=alloc)
          if (alloc/=0) then
      ...
      do  i=1,total_parameters
        if (ivar(i) > 0) then
          ...
          do iobs = 1,fitting%Nenergies
             ...
             !
             enerright(iobs) = calc_(nrot,ipar_)%energy(N)
             ! 
          enddo 
          !
          potparam(i)=tempx-deltax
          ...
          do iobs = 1,fitting%Nenergies
             ...
             !
             enerleft(iobs) = calc_(nrot,ipar_)%energy(N)
             ! 
          enddo 
          !
      ...
      do irot = 1,Nrot
        do ipar_ = 1,2
          !
          if (associated(calc_(irot,ipar_)%energy)) deallocate(calc_(irot,ipar_)%energy)
          !
        enddo 
      enddo
      !
...
!------------------------------------------!
    ...
    subroutine get_vibronic_energies(Nmax,Nstates,calc)
      ...
      integer(ik),intent(inout)  :: Nstates(0:Nmax,2)
      type(calcT),intent(inout) :: calc(0:Nmax,2)
      !
      integer(ik)              :: tunit,tau,KcPot,Kma,ISR,J2,ipar,Nrot,Ntot,Nsize,nn
      integer(ik)    :: alloc,i
      real(rk)  :: Energy,De,wei,Ome,Wome,Wma,Jrot,irot
      character(len=3)   :: Ka
      character(len=5)   :: Jch
      integer(ik)  :: Nstates_(0:Nmax,2)
      !
      tunit = 22
      !
      ...
      if (Ntot==0) then 
         !
         do 
           read(tunit,"(a5,2x,i2,1x,i5,1x,f12.4)",end=119) Jch,tau,nn,Energy ! ,De,V,Ka,Kc,v1,v2,v3,Wei,Ome,Wome,Pot,Kma,Wma,ISR,  N
           !
           do i=1,len_trim(Jch)
               if (Jch(i:i) == "/") Jch(i:i) = " "
           end do
           ...
           if (Nrot>Nmax) cycle
           !
           Nstates_(Nrot,ipar) = Nstates_(Nrot,ipar) + 1
           !
           cycle
           !         
         119 continue
         ...
         do irot = 0,Nrot
           do ipar = 1,2
             !
             Nsize = max(1,Nstates(irot,ipar))
             !
             allocate(calc(irot,ipar)%energy(Nsize),stat=alloc)
             if (alloc/=0) then
      ...
      !
      do 
        read(tunit,"(a5,2x,i2,1x,i5,1x,f12.4)",end=121) Jch,tau,nn,Energy ! De,V,Ka,Kc,v1,v2,v3,Wei,Ome,Wome,Pot,Kma,Wma,ISR,  N
        !
        do i=1,len_trim(Jch)
            if (Jch(i:i) == "/") Jch(i:i) = " "
        end do
        ...
        Nstates_(Nrot,ipar) = Nstates_(Nrot,ipar) + 1
        !
        nn = Nstates_(Nrot,ipar)
        !
        calc(Nrot,ipar)%energy(nn) = Energy
        !
        cycle
        !         
      121 continue
